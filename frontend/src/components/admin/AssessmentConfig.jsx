...
const AssessmentConfig = () => {
  ...
  const [components, setComponents] = useState([{ name: '', type: 'exam', weightage: 0, maxScore: 100 }]);
        ...
        <div className="card" style={{marginTop:'1rem'}}>
          ...
          {components.map((comp, idx) => (
            <div key={idx} className="form-grid" style={{marginBottom:'0.5rem'}}>
              <input placeholder="Component Name (e.g., Exam)" value={comp.name} onChange={e => updateComponent(idx, 'name', e.target.value)} />
              ...
              <input type="number" placeholder="Weight (%)" value={comp.weightage} onChange={e => updateComponent(idx, 'weightage', Number(e.target.value))} />
              <input type="number" placeholder="Max Score" value={comp.maxScore} onChange={e => updateComponent(idx, 'maxScore', Number(e.target.value))} />
          ...
          <button className="btn btn-primary" style={{marginLeft:'1rem'}} onClick={handleSave} disabled={components.some(c=>!c.name || c.weightage<=0)}>Save Configuration</button>
        </div>
      )}
    </div>
  );
